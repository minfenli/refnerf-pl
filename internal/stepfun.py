...
def integrate_weights(w):
    ...
    cw0 = torch.cat(
        [torch.zeros(shape, device=w.device), cw, torch.ones(shape, device=w.device)], dim=-1)
    return cw0
...
def sample(
    ...
    if deterministic_center:
        pad = 1 / (2 * num_samples)
        u = torch.linspace(pad, 1. - pad - eps, num_samples, device=w_logits.device)
    else:
        u = torch.linspace(0, 1. - eps, num_samples, device=w_logits.device)
...
def weighted_percentile(t, w, ps):
    ...
    def fn(cw_i, t_i):
        return math.interp(torch.tensor(ps, device=cw_i.device) / 100, cw_i, t_i)
